...
if ($result->num_rows > 0) {
  ...
  // Carrega o conteúdo HTML da inexistenciaDeParentesco.html
  $inexistenciaDeParentescoHTML = file_get_contents('C:\xampp\htdocs\Teste\AdmissaoDigital\src\DocumentacaoHTML\inexistenciaDeParentesco.html');

  // Substitui os marcadores pelos dados do usuário
  $inexistenciaDeParentescoHTML = str_replace(
    array('{nome}', '{cpf}', '{cargo}', '{data}'),
    array($nome, $cpf, $cargo_funcao_label, $currentDate),
    $inexistenciaDeParentescoHTML
  );
  ...
  echo json_encode(array("html" => $inexistenciaDeParentescoHTML));
